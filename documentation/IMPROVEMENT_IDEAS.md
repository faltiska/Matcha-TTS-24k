# Short phonemes errors 
1. The Architectural Fix: Gaussian Upsampling
Even if you train a model to predict a fractional duration (like 1.2), Matcha-TTS (and FastSpeech 2) uses a standard discrete Length Regulator during inference. 
It essentially rounds the prediction and duplicates the hidden states (e.g., repeating the "A" phoneme 1 time, or 2 times). You can replace this.
How it works: Instead of discrete repetition, use Gaussian Upsampling (introduced in Non-Attentive Tacotron).
The Mechanism: Each phoneme is assigned a continuous duration prediction, which is used to define the center and width of a Gaussian distribution along the time axis. The final speech frame is generated by taking a weighted sum of the nearby phonemes based on overlapping bell curves.
The Result: Phonemes blend smoothly into one another. If a phoneme represents 1.2 frames, its influence "smears" exactly 1.2 frames wide, naturally bleeding a 0.2 fraction of its representation into the adjacent frame.
